...
class MemVar(nn.Module):
    ...
    def forward(self,x):
        ...
        for t in range(T):
            ...
            is_val_last = (self.combined and t>=2 and t%3==2 and t<3*N)
            if is_val_last:
...
def run(seed, exempt, steps=3000, tag=''):
    torch.manual_seed(seed); rng=random.Random(seed)
    m=Mod(design='B', exempt_combined=exempt); opt=torch.optim.AdamW(m.parameters(),lr=3e-4)
    for step in range(steps):
        ...
        if (step+1)%1000==0:
            print(f'[{tag}] step {step+1} loss {loss.item():.3f}',flush=True)
    print(f'[{tag}] FINAL:',flush=True)
    eval_per_pos(m)

print('=== B fixed: exempt_combined=True (skip usage_decay for is_val_last), seed 42 ===')
run(42, True, tag='exempt42')
