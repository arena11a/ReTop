...
class SelectiveSSM(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.ln is not None:
            x = self.ln(x)
        B, T, D = x.shape
        dt = F.softplus(self.delta_proj(x))                   # (B, T, dim)
        dt = dt.unsqueeze(-1)                                 # (B, T, dim, 1)
        Bm, Cm = self.in_proj(x).chunk(2, dim=-1)             # (B, T, state) each
        A = -torch.exp(self.A_log)                            # (dim, state)
        dA = torch.exp(A.unsqueeze(0).unsqueeze(0) * dt)      # (B, T, dim, state)
        dB = dt * Bm.unsqueeze(-2)                            # (B, T, dim, state)
        h = torch.zeros(B, D, self.state_dim, device=x.device)
        ys = []
        for t in range(T):                                    # sequential scan (roadmap #5)
            h = dA[:, t] * h + dB[:, t]                       # (B, dim, state)
            y = (h * Cm[:, t].unsqueeze(-2)).sum(-1) + self.D * x[:, t]
            ys.append(y)
        y = torch.stack(ys, 1)                                # (B, T, dim)
...
class SparseConditionalCompute(nn.Module):
    ...
    def __init__(self, dim, n_experts, top_k, key_dim=16, aux_coef=0.1):
        super().__init__()
