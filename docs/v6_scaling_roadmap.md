...
| M2 | **HF packaging** ✅ 2026-08-26 | `HMN3Config`/`HMNForCausalLM` (PreTrainedModel + GenerationMixin), save/from_pretrained round-trip, CausalLMOutput; tokenizer parity with native tokenizers lib; blended logits via _blended_logprobs scatter from IRStats histograms | `SFTTrainer` trains 100 steps on toy data without custom loop (loss 2.22→0.006); outputs identical to native path (blend CE diff 0.0000) — MET |
| M3 | **Precision ladder** ✅ 2026-08-26 | fp32-island audit (log/exp/softmax, gate sigmoid, loss_v33 logaddexp all on float32 inputs); autocast fp16 parity (CPU, max diff 0.12); NaN sweep 5 seeds × 50 steps (zero NaN/inf); mixed-precision backward verified | train parity vs FP32 within tolerance — MET (CPU fp16); full bf16 parity requires GPU (documented for CI) |
| M4 | **Scale spec + smoke** ✅ 2026-08-26 | spec presets: cpu-small (655K), gpu-small (2.2M), gpu-medium (6.0M), gpu-large (14.5M); CPU smoke D=256 1k steps (loss 239.6→0.002, monotonic); tokens/s measured (17 steps/s cpu-small, 6.4 gpu-small@CPU); checkpoint save/resume parity (loss exact match) | loss ↓ monotonically 1k steps — MET; tokens/s reported; ckpt resumable — MET |
| M5 | **Packed sequence** ✅ 2026-08-26 | `hmn/packing.py`: pack_sequences (greedy bin packing + sep tokens), doc_masked_padding (right-pad, attn_mask, position_ids), unpack_outputs (exact round-trip); position_ids reset at doc boundaries (IR compat); ReversibleFunction backward verified with packed sequences | pack/unpack exact, position_ids reset, backward finite — MET; FSDP2/DeepSpeed wrap deferred to GPU CI (honest boundary) |
| M6 | **Streaming data** ✅ 2026-08-26 | `hmn/streaming.py`: StreamJsonlReader (lazy, re-iterable, shard support), BoundedBufferShuffle (reservoir, deterministic per seed), ChatIdConverter (raw→chat-id), InfiniteStreamDataset (torch IterableDataset); bounded buffer shuffle verified (order changed, count preserved, seed-deterministic); DataLoader compat verified; RSS bounded (<64MB over 1k samples) | local jsonl verified — MET; HF Hub streaming deferred to GPU/cluster runners |
| M7 | Kernels — design notes + torch.compile probe ✅ 2026-08-26 (`docs/v6_m7_triton_design.md`, `experiments/v6/m7_compile_probe.py`: parity deterministic ≤9.2e-04 vs dense oracle incl. D768 slice, ≤1e-3 gate MET; 14 IRStats graph breaks; CPU wall-clock too noisy for speedup claims — whole-model compile break-even–1.26×, head-fusion region inconclusive 1.04×/1.44× across runs; Triton kernels pending GPU env) | Triton fused dual-head (gen softmax ⊕ copy ⊕ gate blend in one kernel); optional FA/Mamba kernels for WR | kernel numerics match eager ≤1e-3; end-to-end ≥1.3× decode speedup at D768 (decode criterion flagged at-risk in M7 design §5) |
