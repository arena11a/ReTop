...
| M7 | Kernels — design notes + torch.compile probe ✅ 2026-08-26 (`docs/v6_m7_triton_design.md`, `experiments/v6/m7_compile_probe.py`: 1.70×/1.29× CPU whole-model, head fusion NOT won by compiler at D768); Triton kernels pending GPU env | Triton fused dual-head (gen softmax ⊕ copy ⊕ gate blend in one kernel); optional FA/Mamba kernels for WR | kernel numerics match eager ≤1e-3; end-to-end ≥1.3× decode speedup at D768 (decode criterion flagged at-risk in M7 design §5) |
